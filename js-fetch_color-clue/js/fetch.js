...
export async function fetchNewColor() {
  ...
  try {
    const colorData = await fetch(colorApiUrl);
    const colorObj = await colorData.json();

    console.log(colorObj.hex.value);
    console.log(colorObj.name.value);
    let myColor = setColorToGuess(colorObj.hex.value, colorObj.name.value);
  } catch (error) {
